...
SELECT total.facility_name, total.member_name, total.cost
FROM (SELECT facility_name, member_name, 
CASE WHEN b.memid=0 THEN slots*guestcost
ELSE slots*membercost END AS cost
FROM (SELECT * FROM Bookings WHERE EXTRACT(YEAR FROM starttime)='2012' AND EXTRACT(MONTH FROM starttime)='09' 
AND EXTRACT(DAY FROM starttime)='14') AS b
LEFT JOIN (SELECT memid, CONCAT(firstname, ' ', surname) AS member_name FROM Members) AS m  
ON b.memid=m.memid
LEFT JOIN (SELECT facid, name AS facility_name, membercost, guestcost FROM Facilities) AS f
